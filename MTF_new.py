...
def read_directory(read_path):                                                  #建立功能一:讀取資料夾路徑並導入参数(此處為放置目標路徑read_path)
    ...
    img_list = list(os.listdir(read_path))                                     #listdir( )這個function會找遍傳進去的路徑底下的所有檔案  
    # img_list = sorted(os.listdir(img_path))	                                #文件名按字母排序                                            
    img_list.sort()
    img_list.sort(key = lambda x: int(x[:-36]))                                 #文件名按数字排序並去掉檔名尾巴36個字符
    img_nums=len(img_list)
    path = 'C:/MTF/ROI/'                                                        #创建文件路径
    i = 0                                                                       #設置循環計數器i  
    for i in range(img_nums):
        img_name=img_list[i]   
        if img_name.endswith(".jpg"):                                           #找出自己要的檔案(此處為結尾.bmp的檔案)                                                                            
            print('進入功能一:找到檔案',img_name)                                 #找到檔案後通知以確認有讀取到的顯示訊息
            i = i + 1                                                           #讀取檔案計數增加1次
            img = cv2.imread(read_path + '/' + img_name)                        #讀取目標圖片絕對路徑下的檔案名稱
            cv2.imwrite('C:/MTF/IMG' + '/' +'src_Img'+'%d.bmp'%i , img)         #重新序列檔案名稱後寫入新路徑    
...
def MTF_cal(roi_path ,img_count):
    ...
    for gray_img in os.listdir(roi_path): 
        ...
        cv2.waitKey(10)                                                        # 0代表一直等待按下空白鍵才關閉所有視窗，括号里的数字代表等待多长时间(单位ms)
...
def write_file(MTF_path ,MTF_file ,img_count):
    datas = MTF_file
    with open('C:/MTF/MTF.csv', 'a', newline='') as csvfile:
...
def main():
    ...
    #count_read = 0                                                             #建立計數器
    read_path = 'C:/MTF/TEST'                                                        #读取目標圖文件夹的绝对路径（不能有中文名称）
    MTF_path = 'C:/MTF/MTF.csv'                                                 #預設MTF.csv資料文件路徑
